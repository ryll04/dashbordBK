...
<div class="card" style="max-width: 600px;">
    <form action="{{ route('admin.pelanggan.update', $pelanggan->id_pelanggan) }}" method="POST">
        ...
        <div style="margin-bottom: var(--space-lg);">
            ...
            <input type="text" id="nama_pelanggan" name="nama_pelanggan" class="form-input" value="{{ old('nama_pelanggan', $pelanggan->nama_pelanggan) }}" required>
        ...
        <div style="margin-bottom: var(--space-lg);">
            ...
            <input type="text" id="nomor_telepon" name="nomor_telepon" class="form-input" value="{{ old('nomor_telepon', $pelanggan->nomor_telepon) }}" required>
            @error('nomor_telepon')
                <div style="color: var(--color-error); font-size: 12px; margin-top: 4px;">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div style="margin-bottom: var(--space-lg);">
            ...
            <textarea id="alamat" name="alamat" class="form-input" rows="3">{{ old('alamat', $pelanggan->alamat) }}</textarea>
            @error('alamat')
                <div style="color: var(--color-error); font-size: 12px; margin-top: 4px;">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="flex gap-lg" style="margin-bottom: var(--space-xl);">
            <div style="flex: 1;">
                ...
                <select id="jenis_pelanggan" name="jenis_pelanggan" class="form-input" required>
                    <option value="baru" {{ old('jenis_pelanggan', $pelanggan->jenis_pelanggan) == 'baru' ? 'selected' : '' }}>Baru</option>
                    <option value="lama" {{ old('jenis_pelanggan', $pelanggan->jenis_pelanggan) == 'lama' ? 'selected' : '' }}>Lama</option>
                </select>
            ...
            <div style="flex: 1;">
                ...
                <input type="text" id="catatan" name="catatan" class="form-input" value="{{ old('catatan', $pelanggan->catatan) }}">
                @error('catatan')
                    <div style="color: var(--color-error); font-size: 12px; margin-top: 4px;">{{ $message }}</div>
                @enderror
            </div>
